...
export default function AddMarkerModal({ coords, onConfirm, onCancel }) {
    ...
    <div className={styles.overlay} onClick={onCancel}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          ...
          <span className={styles.headerLabel}>NEW MARKER</span>
            ...
            <div className={styles.colorRow}>
              {COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  title={c.label}
                >
                  <span className={styles.colorDot} style={{ background: c.value, boxShadow: color === c.value ? `0 0 8px ${c.value}` : 'none' }} />
                  <span className={styles.colorLabel} style={{ color: color === c.value ? c.value : 'var(--text-muted)' }}>{c.label}</span>
                </button>
              ))}
            </div>
          </div>
